...
# Railway / GitHub environment variables
...
# Gemini client is created only after configuration has been checked.
gemini_client = None
...
def get_news(asset):
    ...
    try:
        r = httpx.get(
            url,
            headers={"User-Agent": "Sparks-Forex/1.0"},
            timeout=10,
        )
        r.raise_for_status()
...
async def ai_analyze(asset, snap, news, session):
    ...
    schema = {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["BUY", "SELL", "WAIT", "HOLD", "EXIT_WATCH"],
            },
    ...
    try:
        ...
        if s.get("trade"):
            ...
            text = (
                ...
                f"Suggested Amount: <b>₹{amount:,.2f}</b>\n"
            ...
            text = (
                ...
                f"Risk: <b>{s['risk']:.2f}%</b>\n"
                f"Signal: <b>{result.get('action','WAIT')}</b>\n\n"
...
async def text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "budget":
        try:
            ...
            await update.message.reply_text(
                "Enter a valid positive budget, e.g. 10000."
            )
            return
    ...
    if state == "risk":
        try:
            ...
            await update.message.reply_text(
                "Enter a risk rate between 0.1% and 10%, e.g. 1."
            )
            return
    ...
    if state == "trade":
        ...
        await update.message.reply_text(
            "💼 <b>Trade monitoring activated.</b>\n"
            "Monitoring now has priority.",
            parse_mode="HTML",
            reply_markup=trade_buttons(),
        )
        return
    ...
    if d == "start":
        ...
        await q.message.reply_text(
            (
                ...
                f"Risk: {s['risk']:.2f}%\n"
                f"Active Trade: {'YES' if s.get('trade') else 'NO'}"
            )
            if s
            else "No active session.",
            parse_mode="HTML" if s else None,
            reply_markup=menu(),
        )
        ...
        await q.message.reply_text(
            "⚠️ <b>Risk Rate</b>\n\n"
            "Enter maximum risk per trade, e.g. <code>1</code>.",
            parse_mode="HTML",
        )
